        ...
        <el-table v-loading="Loading" :data="sysLoginInfoList" border stripe style="width: 100%"
            ...
            <el-table-column label="登录状态" prop="loginStatus">
                <template slot-scope="scope">
                    <el-tag v-if="scope.row.loginStatus === 1" type="success">成
                        功</el-tag>
            ...
            <el-table-column label="更多操作">
                <template slot-scope="scope">
                    <el-button size="small" type="text" icon="el-icon-delete" @click="handleDelete(scope.row.id)">删
                        除
                    </el-button>
                </template>
            </el-table-column>
    ...
    methods: {
        ...
        async getSysLoginInfoList() {
            ...
            const { data: res } = await
                this.$api.querySysLoginInfoList(this.queryParams)
            if (res.code !== 200) {
        ...
        async handleDelete(id) {
            const confirmResult = await this.$confirm('是否确认删除登录日志编号为"' + id +
                '"的数据项？', '提示', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                type: 'warning'
        ...
        async batchHandleDelete() {
            ...
            const confirmResult = await this.$confirm('是否确认删除登录日志编号为"' +
                loginInfoIds + '"的数据项？', '提示', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                type: 'warning'
            }).catch(err => err)
