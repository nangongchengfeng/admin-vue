<template>
    <el-card>
        ...
        <el-table v-loading="Loading" :data="sysOperationLogList" border stripe style="width: 100%"
            ...
            <el-table-column label="更多操作">
                <template slot-scope="scope">
                    <el-button size="small" type="text" icon="el-icon-delete" @click="handleDelete(scope.row.id)">删除
                    </el-button>
                </template>
            </el-table-column>
        </el-table>
        ...
        <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange"
            :current-page="queryParams.pageNum" :page-sizes="[10, 50, 100, 500,
                1000]" :page-size="queryParams.pageSize" layout="total, sizes, prev, pager, next, jumper"
            :total="total">
        </el-pagination>
    </el-card>
</template>
    ...
    methods: {
        ...
        async getSysOperationLogList() {
            ...
            const { data: res } = await
                this.$api.querySysOperationLogList(this.queryParams)
            if (res.code !== 200) {
        ...
        async handleDelete(id) {
            const confirmResult = await this.$confirm('是否确认删除操作日志编号为"' + id +
                '"的数据项？', '提示', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                type: 'warning'
        ...
        async batchHandleDelete() {
            ...
            const confirmResult = await this.$confirm('是否确认删除操作日志编号为"' +
                sysOperationLogIds + '"的数据项？', '提示', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                type: 'warning'
            }).catch(err => err)
            ...
            const { data: res } = await
                this.$api.batchDeleteSysOperationLog(sysOperationLogIds)
            if (res.code !== 200) {
                this.$message.error(res.msg)
                } else {
                this.$message.success('删除成功')
